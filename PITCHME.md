...
- 定常進行波はクノイド波または孤立波になる。
- しかし、一般に、任意の初期条件から出発した解は必ずしもこれらの定常波解に漸近するとは限らない。
- ザブスキーとクラスカルはかなり広い範囲の初期値に対して解が孤立波解の集まりに漸近することを示した。

---
## KdV方程式の解

- 初期条件が $u\left(x,0\right)=\cos\left(\pi x\right)$ のとき、以下の式の解は $u=\cos\left[\pi\left(x-ut\right)\right]$
`\[
...
\]`
- 波形は時間とともに変形し、時刻 $t=t_{B}=1/\pi$ のとき、点 $x=1/2$ において勾配 $\partial u/\partial x$ が負の無限大となる。この状態をカタストロフという。
- この状態は物理的意味を失うため$\mu\neq0$である必要がある。

---
## KdV方程式

- $t=0$ の初期値で始まった解は、 $t=t_B$ において突っ立ちに近い状態になった。
- しかし、 $t=t=3.6t_B$ においては、波はほとんど完全に分離した孤立波の列となった。
- そして、それらの孤立波は定常解のものと全く同一のものであることが確かめられた。

---
## KdV方程式

- サブスキー(1967)は初期条件として以下の式の孤立波をとり、数値計算をした。
`\[
u\left(x,0\right)=U\mathrm{sech}^{2}\left(\dfrac{x}{D}\right),D=\sqrt{\dfrac{12\mu}{U}}
\]`
- KdV方程式の孤立波解が、衝突によってその特性を失わず、あたかも独立の粒子のような振る舞いをすることをソリトンと呼ぶ。
- 水の波に限らず、プラズマ、格子振動、さらに素粒子のモデルなど多方面の分野で見出されている。
